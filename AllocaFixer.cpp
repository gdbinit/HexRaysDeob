...
struct AllocaFixer : minsn_visitor_t
{
	...
	int visit_minsn(void)
	{
		...
		if (curins->d.t != mop_f)
		{
			msg("[E] %llu: Call to alloca()'s d operand was unexpectedly %s\n", (uint64_t)curins->ea, mopt_t_to_string(curins->r.t));
			return 0;
		}
...
if (func == NULL)
		{
			msg("[E] %llu: curins->d.f was NULL?", (uint64_t)curins->ea);
			return 0;
		}
...
void FixCallsToAllocaProbe()
{
	...
	while (bFirst ? xr.first_to(eaAlloca, XREF_FAR) : xr.next_to())
	{
		...
		if (f == NULL)
		{
			msg("[E] Call to alloca from %llu is not within a function; will not be processed\n", (uint64_t)xr.from);
			continue;
		}
	...
	for (auto f : funcsCallingAlloca)
	{
		...
		if (mba == NULL)
		{
			msg("[E] FixCallsToAllocaProbe(%llu): decompilation failed (%s)\n", (uint64_t)f->start_ea, hf.desc().c_str());
			continue;
		}
		...
		for (auto g : af.m_FixupLocations)
		{
			...
			msg("[I] Adding auto stack point at %llu: %d\n", (uint64_t)eaNext, -g.second);
			// ... fix its stack pointer differential
			if (!add_auto_stkpnt(f, eaNext, -g.second))
			{
				msg("[E] Couldn't change stack delta to %d at %llu\n", -g.second, (uint64_t)g.first);
				// YOLO
